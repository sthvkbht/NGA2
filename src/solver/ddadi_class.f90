...
contains
  ...
  subroutine ddadi_solve(this)
    ...
    if (.not.this%setup_done) call die('[ddadi solve] Solver has not &
         &been setup.')
  ...
  subroutine ddadi_log(this)
    ...
    if (this%cfg%amRoot) then
      write(message,'("ddadi solver [",a,"] for config [",a,"]")')    &
  ...
  subroutine ddadi_print(this)
    ...
    if (this%cfg%amRoot) then
      write(output_unit,'("ddadi solver [",a,"] for config [",a,"]")') &
        trim(this%name), trim(this%cfg%name)
    end if
  ...
  subroutine ddadi_print_short(this)
    ...
    if (this%cfg%amRoot) write(output_unit,'("ddadi solver [",a16,"] &
